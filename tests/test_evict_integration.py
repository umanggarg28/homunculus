...
def _stub_llm_with_tool_call(history, tool_schemas, model=None, tool_choice="auto", reasoning_effort="low"):
...
def test_mid_loop_eviction_keeps_per_call_input_bounded(monkeypatch):
    ...
    def stub(history, tool_schemas, model=None, tool_choice="auto", reasoning_effort="low"):
        iter_counter["n"] += 1
        if iter_counter["n"] >= 6:
