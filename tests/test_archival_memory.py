...
def test_insert_returns_arch_prefixed_token(tmp_path):
    ...
    token = m.archival_memory_insert("hello world", tags=["greeting"])
...
def test_inserted_content_is_retrievable(tmp_path):
    m = _make_memory(tmp_path)
    token = m.archival_memory_insert(
        "The quick brown fox jumps over the lazy dog.",
        tags=["pangram"],
    )
    result = m.archival_memory_search("anything", k=5)
...
def test_empty_archive_returns_friendly_message(tmp_path):
    m = _make_memory(tmp_path)
    result = m.archival_memory_search("anything")
...
def test_search_returns_top_k(tmp_path):
    m = _make_memory(tmp_path)
    for i in range(7):
        m.archival_memory_insert(f"entry number {i}", tags=[f"t{i}"])
    result = m.archival_memory_search("entry", k=3)
...
def test_long_content_preview_is_truncated_with_hint(tmp_path):
    m = _make_memory(tmp_path)
    huge = "X" * 5000
    token = m.archival_memory_insert(huge, tags=["large"])
    result = m.archival_memory_search("anything", k=1)
    assert token in result
...
def test_insert_includes_tags_in_search_output(tmp_path):
    m = _make_memory(tmp_path)
    m.archival_memory_insert("content with tags", tags=["alpha", "beta"])
    result = m.archival_memory_search("content")
    assert "alpha" in result and "beta" in result


def test_insert_handles_no_tags(tmp_path):
    m = _make_memory(tmp_path)
    token = m.archival_memory_insert("no tags here")
    result = m.archival_memory_search("anything")
    assert token in result
...
def test_archival_storage_survives_new_memory_instance(tmp_path):
    ...
    token = m1.archival_memory_insert("persistent content")
    # Fresh instance, same dir
    m2 = _make_memory(tmp_path)
    result = m2.archival_memory_search("anything")
    assert token in result
